...
const HourlyForecast = () => {
    ...
    return (
        <div className={`${
            ...
            <div className='flex lg:flex-row lg:space-x-8 p-3'>
                {days.map((day: any, index: number) => (
                    <button
                        key={index}
                        className={`py-2 px-4 rounded mb-2 lg:mb-0 lg:mr-2 m-2 ${
                            selectedDay === index ? 'bg-blue-500 text-white' : 'bg-gray-200 text-black'
                        }`}
            ...
            <ScrollableList>
                {getEveryFourthHour(days[selectedDay].hour).map((hour: any, index: number) => (
                    <li className='p-4 border-b min-w-max inline-block mb-4 lg:mb-0 lg:ml-0 m-3' key={index}>
