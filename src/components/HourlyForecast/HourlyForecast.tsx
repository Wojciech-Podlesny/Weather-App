...
const HourlyForecast = () => {
    ...
    const getEveryFourthHour = (hours: any[]) => {
        return hours.filter((_, index) => index % 4 === 0).slice(0,3);
    };
    ...
    return (
        <div
            ...
            } col-start-1 col-end-6 row-start-7 row-end-8 lg:col-start-4 lg:col-end-6 lg:row-start-2 lg:row-end-6 rounded-xl p-5 lg:p-0 space-y-4 lg:space-y-0`}>
            <div className='flex lg:flex-row lg:space-x-3  lg:m-5 space-x-2'>
                {days.map((day: any, index: number) => (
                    <span
                        key={index}
                        className={`font-semibold cursor-pointer py-2 px-2 rounded mb-2 lg:mb-0 lg:mr-2 text-xl  ${
                            selectedDay === index
                                ? isDarkMode
